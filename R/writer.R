...
wrSVload <- function(prefix) {
  glue::glue('{prefix}conf = readRDS("{prefix}conf.rds")\n',
             ...
             '{prefix}deg  = readRDS("{prefix}deg.rds")\n',
             '{prefix}info  = readRDS("{prefix}info.rds")\n',
             'cellcluster <- lapply(\n',
             '  {prefix}deg,\n',
...
wrSVmain <- function(prefix, subst = "") {
  glue::glue('optCrt="{{ option_create: function(data,escape) {{return(\'<div class=\\"create\\"><strong>\' + \'</strong></div>\');}} }}" \n', 
             ...
             ' \n',
             '  output$SeuratResults <- downloadHandler( \n',
             '    filename = function() paste0({prefix}info$data_name,".rds"), \n',
             '    content = function(file) file.copy("{prefix}results.rds",file) \n',
             '  ) \n',
             ' \n',
             ' \n',
             '  ### Plots for tab a1 \n',
             ...
             '      "cl", \n',
             '      "Cluster:", \n',
             '      choices = c("All",cellcluster[[input$metaname]]) \n',
             '    ) \n',
             '  }}) \n',
             ...
             '      ), \n',
             '      {{ \n',
             '        deg <- sc1deg[[input$metaname]] \n',
             '        if (input$cl != "All") {{ \n',
             '          deg <- deg[deg$cluster == input$cl,] \n',
             '        }} \n',
             '        deg <- dplyr::filter( \n',
             '          deg, \n',
             '          p_val_adj < input$p_adj, \n',
             '          avg_log2FC > input$log2fc \n',
             '        ) %>% dplyr::select(gene,cluster,pct.1,pct.2,avg_log2FC,p_val_adj) \n',
             '        deg \n',
             '      }} \n',
             '    ) |> \n',
             ...
             '        ) \n',
             '  ) \n',
             '  output$Degxlsx  <- downloadHandler(\n',
             '    filename = function() paste0(sc1info$data_name,"_DEG.xlsx"),\n',
             '    content = function(file) writexl::write_xlsx(sc1deg,path = file)\n',
             '  )\n',
             '   \n',
             '   \n',
             '   \n')
...
wrUIload <- function(prefix) {
  glue::glue('{prefix}conf = readRDS("{prefix}conf.rds")\n',
             '{prefix}def  = readRDS("{prefix}def.rds")\n',
             '{prefix}deg  = readRDS("{prefix}deg.rds")\n',
             'cellcluster <- lapply({prefix}deg, function(x) unique(as.character(x$cluster))) \n',
             '\n\n\n\n')
}
...
wrUImain <- function(prefix, subst = "", ptsiz = "1.25") {
  glue::glue('  ### Tab1.a1: cellInfo vs geneExpr on dimRed \n',
             ...
             '          "cl", \n',
             '          "Cluster:", \n',
             '          c("All",cellcluster[["seurat_clusters"]]) \n',
             '        ) \n',
             '      ), \n',
             '      column( \n',
             '        3, \n',
             ...
             '          "log2fc", \n',
             '          "log2foldchange:", \n',
             '          value = 1,min = 0.25,max = 10,step = 0.01 \n',
             '        ) \n',
             '      ), \n',
